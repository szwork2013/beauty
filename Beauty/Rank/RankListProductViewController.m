...
@interface RankListProductViewController ()

@end

@implementation RankListProductViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *array, NSError *error) {
        if (error) {
            ...
            for (int i = 0; i < array.count; i++) {
                ...
                //        设置tableview代理类
                RankListProductTableViewDataSource *productDataSource = [[RankListProductTableViewDataSource alloc]initWithTableView:tableView classifyId:[array[i] objectId]];
